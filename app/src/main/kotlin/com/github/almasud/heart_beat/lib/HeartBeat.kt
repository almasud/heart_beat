package com.github.almasud.heart_beat.lib
...
import com.github.almasud.heart_beat.camera.CameraSupport
...
open class HeartBeat {
    private val TAG: String = "HeartRateBeat";
    ...
    protected val publishSubject: PublishSubject<Bpm> = PublishSubject.create()
    ...
    fun withAverageAfterSeconds(averageTimer: Int): HeartBeat {
    ...
    private fun start() {
        ...
        context?.get()?.let {
            cameraSupport = CameraSupport().open(0)
        }
    ...
    private fun createCameraPreviewCallback(): Camera.PreviewCallback {
        return object : Camera.PreviewCallback {
            ...
            override fun onPreviewFrame(data: ByteArray?, camera: Camera?) {
                ...
                val low = ((sampleSize * 40).toDouble() / 60.0 / Fs).toFloat().roundToInt()
                val high = ((sampleSize * 160).toDouble() / 60.0 / Fs).toFloat().roundToInt()
                ...
                bpm = (bestI.toDouble() * Fs * 60.0 / sampleSize).toFloat().roundToInt()
                bpmQueue.add(bpm)
    ...
    private fun createCameraPreviewCallback2(): Camera.PreviewCallback {
        ...
        return object : Camera.PreviewCallback {
            ...
            override fun onPreviewFrame(data: ByteArray?, camera: Camera) {
                ...
                val imageAverage = ImageProcessing.decodeYUV420SPtoRedAvg(data.clone(), width, height)
    ...
    fun setFingerDetectionListener(fingerDetectionListener:((Boolean) -> Unit)?): HeartBeat {
